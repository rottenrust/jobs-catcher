...
def test_full_closed_multi_user_flow_and_isolation():
    ...
    assert alice.post("/profile/confirm", headers={"x-csrf-token":csrf}).json()["confirmed"] is True
    ...
    assert alice.post("/worker/run-fixtures", headers={"x-csrf-token":csrf}).status_code == 404
